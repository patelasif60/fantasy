...
class MorePlayerListDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        $playersWeek = $divisionService->getPlayerScoresDivisionWeekWise($division, request()->all());
        $playersMonth = $divisionService->getPlayerScoresDivisionMonthWise($division, request()->all());
        ...
        ->editColumn('position', function ($data) use ($divisionPoints, $division) {
            ...
            $tshirt = strtolower($data->short_code).'_player';
            if ($position == 'GK') {
                $tshirt = strtolower($data->short_code).'_gk';
            }

            $status = '';
            if ($data->player_status) {
                $playerStatus = implode('', explode(' ', trim(strtolower($data->player_status))));
            ...
            return '<div class="player-wrapper js-player-details cursor-pointer" data-id="'.$data->id.'" data-name="'.$data->player_first_name.' '.$data->player_last_name.'" data-club="'.$data->club_name.'"><div><span class="custom-badge custom-badge-lg is-square is-'.strtolower($position).'">'.$position.'</span></div><div> <div class="player-tshirt icon-18 '.$tshirt.' mr-1"></div>'.get_player_name('firstNameFirstCharAndFullLastName', $data->player_first_name, $data->player_last_name).' '.$status.' </div>';
        })
        ->editColumn('m_tshirt_img', function ($data) use ($division) {
            $position = $division->getPositionShortCode(player_position_short($data->position));

            return player_tshirt($data->club_name, $position);
        })
        ...
        ->addColumn('weekPoint', function ($data) use ($playersWeek) {
            return $playersWeek[$data->id]['total'];
        })
        ->addColumn('monthPoint', function ($data) use ($playersMonth) {
            return $playersMonth[$data->id]['total'];
        })
    ...
    public function query(Player $model)
    {
        ...
        $players = $model::leftJoin('fixture_stats as fixture_stats_for_stats', function ($join) use ($season) {
            ...
            $join->whereIn('fixture_stats_for_stats.fixture_id',
                ...
                ->selectRaw('players.id,players.first_name as player_first_name,players.last_name as player_last_name,clubs.id as club_id,clubs.short_code as club_name,player_status.status as player_status,player_status.description as player_status_description,player_contracts.position,teams.id as team_id,teams.name as team_name,users.first_name as user_first_name,users.last_name as user_last_name,transfers.transfer_value as bid,clubs.short_code,team_player_contracts.id as team_player_contract_id,
        ...
        return $players->groupBy('players.id', 'clubs.id', 'clubs.name', 'player_status.status', 'player_status.description', 'player_contracts.position', 'teams.id', 'teams.name', 'transfers.transfer_value', 'clubs.short_code', 'team_player_contracts.id');
    }
}
