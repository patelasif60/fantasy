...
class UpdateFixturePlayerStatsCmd extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'update:fixture-stats-points {fixture_stats}';
    ...
    public function handle()
    {
        ...
        if ($teamPlayerContracts->count() > 0) {
            foreach ($teamPlayerContracts as $contract) {
                $recalculate = $this->service->updateStatsPoints($fixture_stats, $contract['team_id']);
                $recalculateRanking = $this->service->updateStatsRankingPoints($fixture_stats, $contract['team_id']);
