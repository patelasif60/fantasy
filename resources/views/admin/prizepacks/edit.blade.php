...
@push('page-scripts')
<script src="{{ asset('js/admin/prizepacks/edit.js') }}"></script>
@endpush

@section('content')

<form class="js-prize-pack-edit-form" action="{{ route('admin.prizepacks.update', ['prizePack' => $prizePack]) }}" method="post" enctype="multipart/form-data">
    {{ method_field('PUT') }}
    {{ csrf_field() }}
    <div class="block">
            ...
            <div class="row">
                <div class="col-xl-6">
                    <div class="form-group{{ $errors->has('name') ? ' is-invalid' : '' }}">
                        ...
                        <input type="text" class="form-control" id="name" name="name" placeholder="Prize Pack name" value="{{ $prizePack->name }}">
                        @if ($errors->has('name'))
                ...
                <div class="col-xl-6">
                    <div class="form-group">
                        ...
                        <div class="input-group mb-2">
                            ...
                            <input type="text" class="form-control" id="price" name="price" placeholder="Price" value="{{ $prizePack->price }}">
                        </div>
                    </div>
                </div>
                <div class="col-xl-6">
                    <div class="form-group{{ $errors->has('short_description') ? ' is-invalid' : '' }}">
                        ...
                        <textarea class="form-control" id="short_description" rows="3" name="short_description">{{ $prizePack->short_description }}</textarea>
                    </div>
                </div>
                <div class="col-xl-6">
                    <div class="form-group{{ $errors->has('long_description') ? ' is-invalid' : '' }}">
                        <label for="long_description">Long description:</label>
                        <textarea class="form-control" id="long_description" rows="3" name="long_description">{{ $prizePack->long_description }}</textarea>
                    </div>
                </div>
                <div class="col-xl-6">
                    <div class="form-group{{ $errors->has('is_enabled') ? ' is-invalid' : '' }}">
                        ...
                        <select class="form-control js-select2" id="is_enabled" name="is_enabled">
                            @foreach($yesNo as $key => $value)
                                <option value="{{ $key }}" @if($key == $prizePack->is_enabled) selected  @endif>{{$value}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-group{{ $errors->has('badge_color') ? ' is-invalid' : '' }}">
                        ...
                        <select class="form-control js-select2" id="badge_color" name="badge_color">
                            @foreach($badgeColors as $key => $value)
                                <option value="{{ $key }}" {{ $key == $prizePack->badge_color ? 'selected' : '' }}>{{$value}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-group{{ $errors->has('is_default') ? ' is-invalid' : '' }}">
                        <div class="row no-gutters items-push">
                            <label class="css-control css-control-primary css-checkbox">
                                <input type="checkbox" class="css-control-input" name="is_default" {{ $prizePack->is_default == 1 ? 'checked' : '' }}>
    ...
    <div class="form-group">
        <button type="submit" class="btn btn-hero btn-noborder btn-primary">Update prize pack</button>
