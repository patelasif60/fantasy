    ...
    <div class="modal-content">
        <div class="block block-themed block-transparent mb-0">
            ...
            <div class="block-content">
                <form class="js-divison-team-create-form" id="js-divison-team-create-form" action="{{ route('admin.division.team.update', ['divisonteam'=>$divisonteam]) }}" method="post">
                    {{ csrf_field() }}
                    <div class="block">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="season_id" id="filter-season_id" class="form-control js-select2" style="width: 100%">
                                                ...
                                                @foreach($seasons as $id => $season)
                                                    <option value="{{ $id }}" @if($id == $divisonteam->season_id) selected @endif>{{ $season }}</option>
                                                @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="team_id" id="filter-team_id" class="form-control js-select2" style="width: 100%">
                                                ...
                                                @foreach($teams as $id => $team)
                                                    <option value="{{ $team->id }}" @if($team->id == $divisonteam->team_id) selected @endif>{{ $team->name }} ({{$team->consumer->user->first_name.' '.$team->consumer->user->last_name}})</option>
                                                @endforeach
                                        </select>
                                    </div>
                                </div>
                                <input type="hidden" name="division_id" value="{{$divisonteam->division_id}}">
                            </div>
                        </div>
