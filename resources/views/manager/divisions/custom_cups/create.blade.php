...
<script type="text/javascript" src="{{ asset('js/manager/divisions/custom_cups/create.js') }}"></script>
@endpush

@section('page-name')
    <span class="js-cup-title">New cup</span>
@endsection

@section('right')
    <div class="js-main-custom-cup" data-round="{{ route('manage.division.custom.cups.round', ['division' => $division ]) }}">
        <form action="{{ route('manage.division.custom.cups.store',['division' => $division]) }}" method="POST" class="js-custom-cup-create-form">
            @csrf
            <div class="js-step-one js-step">
                <div class="form-group {{ $errors->has('name') ? ' is-invalid' : '' }}">
                    <label for="name">Cup name</label>
                    <div class="row gutters-sm">
                        <input type="text" class="form-control" id="name" name="name" value="" placeholder="e.g. My Custom Cup">
                        @if ($errors->has('name'))
                            ...
                            <div class="team-management-block">
                                ...
                                <div class="team-detail-wrapper d-flex justify-content-between align-items-center flex-grow-1">
                                    ...
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" class="custom-control-input js-cup-teams" value="{{ $team->id }}"  checked="checked" name="teams[]" id="team-{{$team->id}}">
            ...
            <div class="js-step-three js-step d-none">
                ...
                <div class="mt-3 mb-5">
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" checked="checked" class="custom-control-input js-first-round-bye" name="is_bye_random" value="0" id="is_bye_random">
                        <label class="custom-control-label" for="is_bye_random">Random selection</label>
                    </div>
                </div>

                <div class="js-bye-teams-list">
                    <div class="form-group">
                        @foreach($teams as $team)
                            <p>
                                <div class="custom-control custom-checkbox js-bye-team-checkbox" id="js_bye_team_checkbox_{{ $team->id }}">
                                    <input type="checkbox" class="custom-control-input" value="{{ $team->id }}"  name="bye_teams[]" id="bye-teams-{{$team->id}}">
            ...
            <div class="js-step-four js-step d-none">
                The <span class="js-cup-name">Magic cup </span> has been created. Please check the details below. You can edit your cup until the first week of Round 1.
                <div class="mt-2">
