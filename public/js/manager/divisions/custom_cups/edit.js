...
var isTeamChange = false;
var globalDataStep = '';
var globalDataRound = '';

var ManagerCustomCupUpdate = function() {

    var initFormValidations = function () {
        $('.js-custom-cup-update-form').validate(Global.buildValidateParams({
            rules: {
                'name': {
                    required: true,
                },
    ...
    var previousButtonProcess = function () {
        $(document).on('click','.js-cup-back-url',function(){
            ...
            if(step > 1 && step <= 4) {
                ...
                if(step === 2) {
                    if(round === 1) {
                        $('.js-cup-title').html(customCupName);
    ...
    var nextButtonProcess = function () {
        $(".js-next-step").click(function(event) {
            ...
            if(step === 2 || step === 4) {
                if (!$(".js-custom-cup-update-form").valid()) {
                    return false;
                }
            }
            scrollAnimation();
                ...
                if(round <= getRounds()) {
                    ...
                    if(byesCount > 0) {
                        ...
                        if(isTeamChange) {
                            ...
                            if(!Site.selectedTeamsBye.length) {
    ...
    var generateRoundFromAjax = function() {
            $.ajax({
                url: $('.js-main-custom-cup').data('round')+"?rounds="+getRounds()+'&cup='+customCupId,
                type: 'GET',
                dataType: 'html',
            })
    ...
    var initFormSubmit = function () {
        $(".js-custom-cup-update-form").submit(function(){
            $('.js-step-submit').attr("disabled", true);
        });
    }

    var isTeamsChangeDetect = function () {
        $('.js-team-checkbox-select').click(function(event) {
            isTeamChange = true;
        });
    }
